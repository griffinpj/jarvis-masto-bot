...
let masto_stream = null;

const listenToStream = (stream) => {
    ...
    if (ws) {
        // Listen for ws stream connection to close
        ws.on('close', async () => {
            logger.error('WS Stream connection closed unexpectedly');
            logger.info('Reconnecting to stream');
            masto_stream = await mastodon.stream.user();
            listenToStream(masto_stream);
            //setTimeout(listenToStream(masto_stream), 1000);
        });
    }
}
